...
with tf.name_scope("Generator"):
    def generator(y, z_dim):
        ...
        gen_biases = {
            'b0': tf.get_variable('g_b0', [z_dim], initializer=tf.truncated_normal_initializer()),
        ...
        g0 = tf.matmul(y, gen_weights['w0']) + gen_biases['b0']

        g1 = tf.matmul(g0, gen_weights['w1']) + gen_biases['b1']
...
gen_sample = generator(y, z_dim)  # Generated Images.
...
with tf.name_scope("summary"):
    ...
    images_for_tensorboard = generator(y, z_dim)
    tf.summary.image('Generated_images', images_for_tensorboard, 5)
    summary_merged = tf.summary.merge_all()

logdir = "tensorboard_cgan_mnist/" + datetime.datetime.now().strftime("%Y%m%d-%H%M%S")
    ...
    with tf.Session() as sess:
        ...
        for i in range(1):
            ...
            if i % 1 == 0:
                ...
                sample_output = sess.run(gen_sample, feed_dict={y: label})
                summary = sess.run(summary_merged, {y: label, x: data})
